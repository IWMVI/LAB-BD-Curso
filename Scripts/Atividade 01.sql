...
FROM
	professor p
JOIN profturma p2 ON
	p2.CodProf = p.CodProf
WHERE
	p.CodDepto = 'INF01'
	AND p2.AnoSem = 20021;
...
JOIN profturma p2 ON
	p2.CodProf = p.CodProf
JOIN depto d ON
	d.CodDepto = p2.CodDepto
JOIN disciplina d2 ON
	d2.CodDepto = d.CodDepto
JOIN depto d3 ON
	d2.CodDepto = d3.CodDepto
WHERE
...
FROM
	professor P
JOIN profTurma PT1 ON
	P.CodProf = PT1.CodProf
JOIN horario H1 ON
	PT1.AnoSem = H1.AnoSem
	AND PT1.CodDepto = H1.CodDepto
	AND PT1.NumDisc = H1.NumDisc
	AND PT1.SiglaTur = H1.SiglaTur
JOIN profTurma PT2 ON
	P.CodProf = PT2.CodProf
JOIN horario H2 ON
	PT2.AnoSem = H2.AnoSem
